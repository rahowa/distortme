...
class ApplyAugmentation:
    ...
    def apply_aug(self, image: Image, current_aug: BaseAug) -> Image:
        return current_aug(image=image)["image"]

    def save_image(self, image: Image, path: str) -> None:
        cv2.imwrite(path, image)
    
    def __call__(self, img_path: str) -> Image:
        img_name = img_path.split('/')[-1]
        self.save_image(self.apply_aug(cv2.imread(img_path), self.aug),
