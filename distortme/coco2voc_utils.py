...
from distortme.base_types import Box
...
def base_dict(filename: str,
    ...
    return {
        "annotation": {
            ...
            "segmented": "0",
            "owner": {"name": "unknown"},
            "source": {"database": "The COCO 2017 database",
...
def base_object(size_info: Dict[str, int],
                name: str,
                bbox: Box) -> Dict[str, Any]:
    x1, y1, w, h = bbox
    x2, y2 = x1 + w, y1 + h
...
@print_delimiter("Convert to PASCAL VOC dataset...")
def main_coco2voc(annotations: str, dstdir: Optional[str]) -> None:
    dst_base = os.path.join(dstdir or "data", "VOCdevkitCOCO", "VOCCOCO")
    ...
    ann_splt = os.path.basename(annotations).split('_')
    stage = ann_splt[1] if len(ann_splt) > 0 else ann_splt[0]

    print("Write image sets...")
    result_file = os.path.join(dst_dirs["ImageSets"], "{stage}.txt")
